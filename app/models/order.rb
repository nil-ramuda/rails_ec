...
class Order < ApplicationRecord
  ...
  with_options presence: true do
    ...
    validates :user_name, length: { maximum: 10, message: 'は10文字以内で入力してください' },
                          format: { with: /\A[a-z0-9]+\z/i, message: 'は英文字のみが使えます' }
    validates :email, length: { maximum: 255, message: 'は255文字以内で入力してください' },
                      format: { with: URI::MailTo::EMAIL_REGEXP, message: 'は有効なものを入力してください' }
    ...
    validates :credit_card_number, length: { is: 15, message: 'は15桁で入力してください' },
                                   numericality: { only_integer: true, message: 'は半角数字で入力してください' }
    validates :cvv, length: { is: 3, message: 'は3桁で入力してください' },
                    numericality: { only_integer: true, message: 'は半角数字で入力してください' }
